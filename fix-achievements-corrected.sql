...
ALTER TABLE achievements DROP CONSTRAINT IF EXISTS valid_achievement_type;
ALTER TABLE achievements DROP CONSTRAINT IF EXISTS valid_tournament_rank;

-- Add constraints
ALTER TABLE achievements 
ADD CONSTRAINT valid_achievement_type 
CHECK (achievement_type IN ('tournament_winner', 'top_scorer', 'defensive_leader', 'most_conceded', 'king_of_emotions'));

ALTER TABLE achievements 
ADD CONSTRAINT valid_tournament_rank 
CHECK (
